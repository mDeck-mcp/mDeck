...
pub fn setup_tray(app: &tauri::App) -> tauri::Result<()> {
    let header = MenuItem::new(app, "MCPDeck", false, None::<&str>)?;
    let sep1 = PredefinedMenuItem::separator(app)?;
    let open = MenuItem::with_id(app, "open", "Open MCPDeck", true, None::<&str>)?;
    ...
    let mut builder = TrayIconBuilder::with_id("tray")
        .menu(&menu)
        .tooltip("MCPDeck \u{2014} MCP Server Manager")

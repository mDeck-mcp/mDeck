...
/// One-time rename `~/.mtarsier` → `~/.mcpdeck` if the new dir does not exist yet.
pub fn migrate_legacy_mcpdeck_home() {
    let Some(home) = dirs::home_dir() else {
        return;
    };
    let old = home.join(".mtarsier");
    let new = home.join(".mcpdeck");

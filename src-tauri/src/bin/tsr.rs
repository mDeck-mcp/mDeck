...
#[command(
    name = "tsr",
    about = "MCPDeck CLI — manage MCP servers from your terminal",
    version
)]
struct Cli {
    #[command(subcommand)]
...
enum Commands {
    ...
    /// Disable a server (removes from client config, saves to MCPDeck store)
    Disable {
        name: String,
        #[arg(long)]
        client: String,
...
fn cmd_disable(name: String, client: String) {
    ...
    if let Err(e) = write_store(client_def.id, &store) {
        eprintln!("Error writing MCPDeck store: {}", e);
        process::exit(1);
    }
...
fn cmd_enable(name: String, client: String) {
    ...
    let entry = match store.get(&name) {
        ...
        None => {
            eprintln!("Server '{}' not found in MCPDeck store for {}", name, client_def.name);
            process::exit(1);
        }
    };

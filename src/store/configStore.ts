...
type McpDeckStore = Record<string, ServerEntry>;
...
/**
 * Build the MCPDeck store key for a client + scope.
...
export const useConfigStore = create<ConfigStore>((set, get) => ({
  ...
  loadConfig: async (clientId) => {
    ...
    const loadMcpDeckStore = async (clientServers: Record<string, unknown>) => {
      const key = buildStoreKey(clientId, activeScope);
      const store = await invoke<McpDeckStore>("read_mcpdeck_store", { storeKey: key })
        .catch(() => ({} as McpDeckStore));
      ...
      await invoke("write_mcpdeck_store", { storeKey: key, store }).catch(() => {});
    ...
    try {
      ...
      await loadMcpDeckStore(rawServers);
      get().loadBackups();
    } catch {
      // Config file doesn't exist yet — still load MCPDeck store so disabled/removed servers surface
      set({
        rawContent: "",
        originalContent: "",
        servers: {},
